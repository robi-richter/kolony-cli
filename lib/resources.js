...
// copies the example model file to the destination folder
self.copyModel = function(destination, callback){
  // check if provided path exists
  if (fs.existsSync(destination)){
    callback(new Error('model file already exists!'));
    return;
  }
  ...
  self.fetchModuleSeed(function(error, seedPath){
    ...
    cp(seedPath+"/models/example.js", destination);
    callback();
  })
}

// copies the example controller file to the destination folder
self.copyController = function(destination, callback){
  // check if provided path exists
  if (fs.existsSync(destination)){
    callback(new Error('controller file already exists!'));
    return;
  }
  ...
  self.fetchModuleSeed(function(error, seedPath){
    ...
    cp(seedPath+"/controllers/example.js", destination);
    callback();
  })
}

// copies the example api file to the destination folder
self.copyApi = function(destination, callback){
  // check if provided path exists
  if (fs.existsSync(destination)){
    callback(new Error('api file already exists!'));
    return;
  }
  ...
  self.fetchModuleSeed(function(error, seedPath){
    ...
    cp(seedPath+"/api/example.js", destination);
    callback();
  })
}
