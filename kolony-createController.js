...
program
  ...
  .arguments('<cName> <mName>')
  .action(function (cName, mName) {
...
if (program.module===undefined){
  console.log("please specify a controller name");
  process.exit(0);
}
...
projectController.moduleController.createController(controllerName, modelName, function(error){
  if (error){
    console.log(error);
  }
});
